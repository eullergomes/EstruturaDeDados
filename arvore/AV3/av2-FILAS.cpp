...
Paciente *next (Filas *filas){
    ...
    //1 - prioridade
    if (filas->emergencia != NULL){ //se não tiver ninguem na fila de emergencia
        return removerDaFila(&(filas)->emergencia);
    }
    //2 - prioridade
    if(cont != 3 && filas->urgencia != NULL){ //tem que ser difente de 3 e tem que ter alguém na fila
        cont++;
